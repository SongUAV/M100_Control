...
public:
  ...
  void setupVideo() {
    ...
    m_cap >> frame;
    frame_width=   m_cap.get(CV_CAP_PROP_FRAME_WIDTH);
    frame_height=   m_cap.get(CV_CAP_PROP_FRAME_HEIGHT);
  ...
  void print_detection(AprilTags::TagDetection& detection)  {
    ...
    if (detection.id==AT_ID_L){
      ...
      //cout << "ATpos_x_L" <<ATpos_x_L
      //<< "ATpos_y_L" <<ATpos_y_L
      //<< "ATpos_z_L" <<ATpos_z_L <<endl;
    ...
    if (detection.id==AT_ID_S){
      ...
      cout << "ATpos_x_S" <<ATpos_x_S
      << "ATpos_y_S" <<ATpos_y_S
      << "ATpos_z_S" <<ATpos_z_S <<endl;
    }
...
void* loop( void *reforges) {
    //int image_count =0;
  AprilTAG *mydata;
  mydata=(AprilTAG *)reforges;
  cv::Mat image;
  cv::Mat image_gray;
...
void* grabpictures(void *reforges){
  AprilTAG *mydata;
  mydata=(AprilTAG *)reforges;
  ...
  while(!mydata->thread_kill){
    ...
    /*if (mydata->m_cap.read(image)){
    	 mydata->video.write(image);
...
int precision_landing(CoreAPI* api,Flight* flight, AprilTAG* reforges, float descending_speed, int blockingTimeout, int trylanding){
  int landed=0;
  reforges->fs << api->getBroadcastData().pos.height << endl;
  // height greater than 4m, descend
  if (api->getBroadcastData().pos.height>4.0){
    moveByPositionOffset(api, flight,0,0,descending_speed,0, blockingTimeout*200,1,5);
    usleep(200000);
    std::cout << "stage 1" << endl;
    reforges->fs << "stage 1" << endl;
  }
  ...
  // check if the AT_L is detected
    if (reforges->detected_L){
      ...
      if (api->getBroadcastData().pos.height>0.5){
      	std::cout << "stage 2" << endl;
        reforges->fs << "stage 2" << endl;
        moveByPositionOffset(api, flight,reforges->ATpos_z_L,reforges->ATpos_y_L, descending_speed,0, blockingTimeout*100,1,5);
        usleep(100000);
      }
      // height less than 0.5m ,check the offset 
        ...
        if (fabs(reforges->ATpos_z_L)>=0.2 || fabs(reforges->ATpos_z_L)>=0.2){
          // ascending to 2m and try landing again
        	std::cout << "ascending 1" << endl;
          reforges->fs << "ascending 1" << endl;
      ...
      std::cout << "AT_L is not been detected" << endl;
  ...
  if (api->getBroadcastData().pos.height <= 0.5){
    ...
    // height greater than 0.2m
    	std::cout << "stage 3" << endl;
      reforges->fs << "stage 3" << endl;
      if (api->getBroadcastData().pos.height>0.3){
        moveByPositionOffset(api, flight, reforges->ATpos_z_S,reforges->ATpos_y_S,descending_speed,0, blockingTimeout*100,1,5);
        usleep(100000);
      }
      // height less than 0.2m ,check the offset 
        ...
        if (fabs(reforges->ATpos_z_S)<=0.1 && fabs(reforges->ATpos_z_S)<=0.1){
          ...
          return landed;
        	...
        	std::cout << "ascending 2" << endl;
          reforges->fs << "ascending 2" << endl;
          moveByPositionOffset(api, flight, reforges->ATpos_z_S,reforges->ATpos_y_S,2.0,0, blockingTimeout*500,1,5);
          usleep(500000);
          trylanding++;
        }
      }
      ...
      std::cout << "AT_S is not been detected" << endl;
      moveByPositionOffset(api, flight,0,0,0,0, blockingTimeout*10,1,5);
      usleep(50000);
...
bool AT_follow(CoreAPI* api,Flight* flight, AprilTAG* reforges, int blockingTimeout,float curr_height){
  bool reached=false;
  reforges->fs << api->getBroadcastData().pos.height << endl;
  ...
  if (reforges->detected_L){
    //reforges->reset();
    if (fabs(reforges->ATpos_z_L)>=1.3||fabs(reforges->ATpos_y_L)>=1.3){
      ...
      moveWithVelocity(api, flight, (reforges->ATpos_z_L)/0.6, (reforges->ATpos_y_L)/0.6, 0, 0 ,  blockingTimeout*100, 1, 0.02);
      //usleep(100000);
      usleep(100000);
      //LS.WriteChar('1');
      std::cout << "AT_follow" << endl;
      reforges->fs << "AT_follow_V" << endl;
      reforges->fs << reforges->ATpos_z_L << endl;
      reforges->fs << reforges->ATpos_y_L << endl;
    }
    else {
      moveByPositionOffset(api, flight, reforges->ATpos_z_L,reforges->ATpos_y_L,curr_height-api->getBroadcastData().pos.height,0, blockingTimeout*100,1,5);
      //moveByPositionOffset(api, flight, reforges->ATpos_z_L,reforges->ATpos_y_L,0,0, blockingTimeout*100,1,5);
      //usleep(100000);
      //usleep(100000);
      //LS.WriteChar('1');
      std::cout << "AT_follow" << endl;
      reforges->fs << "AT_follow_P" << endl;
       reforges->fs << reforges->ATpos_z_L<< endl;
      reforges->fs << reforges->ATpos_y_L << endl;
  ...
  // not detected 
    ...
    //LS.WriteChar('0');
    moveByPositionOffset(api, flight,0,0,curr_height-api->getBroadcastData().pos.height,0, blockingTimeout*100,1,15);
    //moveByPositionOffset(api, flight,0,0,0,0, blockingTimeout*50,1,15);
    usleep(10000);
    std::cout << 1111 << endl;
    reforges->fs << "no at" << endl;
  }
 return reached;
}
...
int main() {
  ...
  std::cout << "11111" << endl;
	...
	//LinuxThread send(api, 1);
          ...
          //usleep(20000);
				std::cout << "height" << api->getBroadcastData().pos.height << endl;
				if (twom==0){
					moveWithVelocity(api, flight, 0, 0, 1, 0 ,  blockingTimeout*1000, 1, 0.02);
					usleep(2000000);
				}
        if(fabs(2.0-api->getBroadcastData().pos.height)>0.05 && twom ==0){
        	std::cout << api->getBroadcastData().pos.height << endl;
          moveWithVelocity(api, flight, 0, 0, -0.5, 0 ,  blockingTimeout*1000, 1, 0.02);
				//moveByPositionOffset(api, flight, 0,0,2.0-api->getBroadcastData().pos.height,0, blockingTimeout*1000,1,3);
				usleep(1000000);
        //  usleep(30000);
        ...
        twom=1;
            ...
            //reforges->fs.open("image_detection.txt", std::fstream::in | std::fstream::out | std::fstream::app);
						reforges->thread_kill = false;
            char filename[100] = {0}; 
            sprintf(filename,"AT_follow_%d%02d%02d%02d%02d%02d.txt",p->tm_year+1900,p->tm_mon+1,p->tm_mday,p->tm_hour,p->tm_min,p->tm_sec); 
						reforges->fs.open(filename, std::fstream::in | std::fstream::out | std::fstream::app);
						int test=0;
            float curr_height=api->getBroadcastData().pos.height;
              ...
              //reforges->fs << reforges->detected_L << endl;
							...
							followed=AT_follow(api,flight, reforges,  blockingTimeout, curr_height);
            ...
            char filename[100] = {0}; 
            sprintf(filename,"precision_landing_%d%02d%02d%02d%02d%02d.txt",p->tm_year+1900,p->tm_mon+1,p->tm_mday,p->tm_hour,p->tm_min,p->tm_sec); 
         ...
         // pthread_create(&processing, NULL, loop,  (void*)reforges);
              ...
              // descending speed, m/time
					float descending_speed =-0.4;

					do{
						//usleep(200000);
						landed= precision_landing(api,flight, reforges,  descending_speed,  blockingTimeout,  trylanding);
						//std::cout << landed << endl;
					//}while(landed!=1 || trylanding<5||api->getBroadcastData().pos.height > 0.12);
					}while(api->getBroadcastData().pos.height > 0.12);
					reforges->fs.close();
					ackReturnData landingStatus = landing(api, flight,blockingTimeout);
