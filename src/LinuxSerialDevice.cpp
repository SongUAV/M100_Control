...
LinuxSerialDevice::LinuxSerialDevice(std::string device, unsigned int baudrate) {
	m_device = device;
  m_baudrate = baudrate;
	m_memLock = (pthread_mutex_t)PTHREAD_MUTEX_INITIALIZER;
	m_msgLock = (pthread_mutex_t)PTHREAD_MUTEX_INITIALIZER;
	m_ackLock = (pthread_mutex_t)PTHREAD_MUTEX_INITIALIZER;
  pthread_cond_init(&m_ackRecvCv, NULL);

/*! These mutexes are used for the non blocking callback ACK mechanism */
	m_nbAckLock = (pthread_mutex_t)PTHREAD_MUTEX_INITIALIZER;
	m_headerLock = (pthread_mutex_t)PTHREAD_MUTEX_INITIALIZER;
