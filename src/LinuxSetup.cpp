...
ackReturnData takeControl(CoreAPI* api)
{
  ...
  switch (takeControlData.ack)
  {
    ...
    case ACK_SETCONTROL_OBTAIN_RUNNING:
      ...
      std::cout <<obtainingControl<< std::endl;
      takeControl(api);
      //usleep(20000);
      break;
    ...
    default:
      {
        ...
        std::cout << takeControlData.ack << std::endl;
      }
      break;
  }
  takeControlData.status = 1;
...
void activateNonBlocking(CoreAPI* api)
{
  ...
  //pointerActivationCallback(CoreAPI* api, Header *protHeader, DJI::UserData data);
